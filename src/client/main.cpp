...
int main(int argc, char **argv) {
  ...
  for (;;) {
    ...
    int choice = 0;
    cin >> choice;
    cin.get(); // 读掉缓冲区残留的回车，以免影响后续字符串的输入
    ...
    case 1: // login业务
    {
      ...
      if (len == -1) { cerr << "send login msg error:" << request << endl; }
...
void addfriend(int clientfd, string str) {
  ...
  if (-1 == len) { cerr << "send addfriend msg error -> " << buffer << endl; }
}
// "chat" command handler
void chat(int clientfd, string str) {
  ...
  int len = send(clientfd, buffer.c_str(), strlen(buffer.c_str()) + 1, 0);
  if (-1 == len) { cerr << "send chat msg error -> " << buffer << endl; }
}
...
void creategroup(int clientfd, string str) {
  ...
  int len = send(clientfd, buffer.c_str(), strlen(buffer.c_str()) + 1, 0);
  if (-1 == len) { cerr << "send creategroup msg error -> " << buffer << endl; }
}
// "addgroup" command handler
void addgroup(int clientfd, string str) {
  ...
  int len = send(clientfd, buffer.c_str(), strlen(buffer.c_str()) + 1, 0);
  if (-1 == len) { cerr << "send addgroup msg error -> " << buffer << endl; }
}
...
void groupchat(int clientfd, string str) {
  ...
  int len = send(clientfd, buffer.c_str(), strlen(buffer.c_str()) + 1, 0);
  if (-1 == len) { cerr << "send groupchat msg error -> " << buffer << endl; }
}
// "loginout" command handler
